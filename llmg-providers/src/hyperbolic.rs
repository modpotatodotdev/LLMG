...
impl HyperbolicClient {
    pub fn from_env() -> Result<Self, LlmError> {
        let api_key = std::env::var("HYPERBOLIC_API_KEY").map_err(|_| LlmError::AuthError)?;
...
impl Provider for HyperbolicClient {
...
mod tests {
    ...
    #[test]
    fn test_hyperbolic_client_creation() {
        let client = HyperbolicClient::new("test-key");
        assert_eq!(client.provider_name(), "hyperbolic");
    }

    #[test]
    fn test_from_env_missing_key() {
        let original = std::env::var("HYPERBOLIC_API_KEY").ok();
        std::env::remove_var("HYPERBOLIC_API_KEY");

        let result = HyperbolicClient::from_env();
        assert!(result.is_err());

        if let Some(key) = original {
            std::env::set_var("HYPERBOLIC_API_KEY", key);
        }
    }
}
