...
impl CerebrasClient {
    /// Create a new Cerebras client from environment
    pub fn from_env() -> Result<Self, LlmError> {
        let api_key = std::env::var("CEREBRAS_API_KEY").map_err(|_| LlmError::AuthError)?;

        Ok(Self::new(api_key))
    }

    /// Create a new Cerebras client with explicit API key
...
impl Provider for CerebrasClient {
...
mod tests {
    ...
    fn test_from_env_missing_key() {
        ...
        let original = std::env::var("CEREBRAS_API_KEY").ok();
        std::env::remove_var("CEREBRAS_API_KEY");

        let result = CerebrasClient::from_env();
