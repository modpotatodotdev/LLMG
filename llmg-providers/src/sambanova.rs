...
impl SambaNovaClient {
    pub fn from_env() -> Result<Self, LlmError> {
        let api_key = std::env::var("SAMBANOVA_API_KEY").map_err(|_| LlmError::AuthError)?;
...
impl Provider for SambaNovaClient {
...
mod tests {
    ...
    #[test]
    fn test_sambanova_client_creation() {
        let client = SambaNovaClient::new("test-key");
        assert_eq!(client.provider_name(), "sambanova");
    }

    #[test]
    fn test_from_env_missing_key() {
        let original = std::env::var("SAMBANOVA_API_KEY").ok();
        std::env::remove_var("SAMBANOVA_API_KEY");

        let result = SambaNovaClient::from_env();
        assert!(result.is_err());

        if let Some(key) = original {
            std::env::set_var("SAMBANOVA_API_KEY", key);
        }
    }
}
