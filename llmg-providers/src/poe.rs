...
impl PoeClient {
    /// Create a new Poe client from environment
    pub fn from_env() -> Result<Self, LlmError> {
        let api_key = std::env::var("POE_API_KEY").map_err(|_| LlmError::AuthError)?;

        Ok(Self::new(api_key))
    }

    /// Create a new Poe client with explicit API key
...
mod tests {
    ...
    #[test]
    fn test_poe_client_creation() {
        let client = PoeClient::new("test-key");
        assert_eq!(client.provider_name(), "poe");
    }

    #[test]
    fn test_from_env_missing_key() {
        let original = std::env::var("POE_API_KEY").ok();
        ...
        if let Some(key) = original {
            std::env::set_var("POE_API_KEY", key);
        }
    }
}
