...
:::tip
The gateway requires an `Authorization: Bearer <token>` header on all requests. The token value is not validated in the current release — any non-empty value works. See the [Authentication guide](/LLMG/gateway/authentication/) for details.
:::
