...
The gateway requires an `Authorization: Bearer <token>` header on all requests (except `/health`). For full details on how gateway auth and provider auth work, see the **[Authentication guide](/LLMG/gateway/authentication/)**.
