...
Key metrics include:
- `llmg_requests_total` - Total requests by provider and status
- `llmg_request_duration_seconds` - Request latency histogram
- `llmg_active_connections` - Current active connections

### Grafana Dashboard

Import the following dashboard JSON to monitor your gateway:
    ...
    "panels": [
      {
        ...
        "targets": [
          {
            "expr": "rate(llmg_requests_total[5m])"
          }
        ]
      }
    ]
